...
void sendRulePacket(snortRule* rule, CURL *handle, std::string host,bool verbose){
    ...
    for(int j=0;j<rule->body.content.size();j++){
    	if(rule->body.negatedContent.at(j)){
    		//skip content because content is negated (and hope it is not generated by accident before)
    	}else{
	...
	if(result != CURLE_OK){
			fprintf(stderr, "curl_easy_perform() failed for packet from rule sid %s, with url %s, with error: %s. (If this happens for an HTTP HEAD request you can safely ignore this error, the packet was sent succesfully.)\n",rule->body.sid.c_str(),hostUri.c_str(), curl_easy_strerror(result));
			return;
	}
}
